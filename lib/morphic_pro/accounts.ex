defmodule MorphicPro.Accounts do
  @moduledoc """
  The Accounts context.
  """

  alias MorphicPro.Repo
  alias MorphicPro.Accounts.{User, UserToken}
  ...
  def update_user_email(user, token) do
    ...
    with {:ok, query} <-
           UserToken.verify_user_change_email_token_query(token, context),
  ...
  def deliver_update_email_instructions(
    ...
    email.(user, update_email_url_fun.(encoded_token))
    |> mailer.()
  end
  ...
  """
  def deliver_user_confirmation_instructions(%User{} = user, confirmation_url_fun, email, mailer)
    ...
    if user.confirmed_at do
      ...
      email.(user, confirmation_url_fun.(encoded_token))
      |> mailer.()
    end
  end
  ...
  def deliver_user_reset_password_instructions(
    ...
    email.(user, reset_password_url_fun.(encoded_token))
    |> mailer.()
  end
